...
info_string3 = "N runs a Nearest Neighbour solver. T brute forces the search. F toggles fast solve (no screen update)"
            ...
            if tsp_ran:
                if solver == 'T':
                    ...
                    for path in itertools.permutations(nodes):
                        ...
                        if round(ran, 2) % 0.5 != 0 and fast_solve:
                            continue
                        ...
                        screen.fill((128, 128, 128), (0, 125, width, 25))
                        fps.render(screen, 0, 125, 'Initial distance: ' + str(round(first_distance, 4)))
                        ...
                        fps.render(screen, width/2, 102, 'Searched paths ' + value + ' times.')
                        fps.render(screen, width/2, 125, 'Distance: ' + str(round(shortest_distance, 4)))
                    ...
                    if tsp_timed_out or exit_tsp:
                        screen.fill((128, 128, 128), (0, 125, width, 25))
                        screen.fill((128, 128, 128), (0, 150, width, 25))
                        screen.fill((128, 128, 128), (0, 125, width, 25))
                        screen.fill((128, 128, 128), (0, 102, width, 25))
                        fps.render(screen, 0, 122, 'TSP: forced exit with best path as ' + str(round(shortest_distance, 4)))
                        tsp_timed_out = False
                        fps.render(screen, width/2, 102, 'Searched paths ' + value + ' times.')
                        fps.render(screen, width/2 + 30, 125, 'Distance: ' + str(round(shortest_distance, 4)))
                        fps.render(screen, 0, 102, 'TSP runtime: ' + str(round(ran, 4)) + 's')
                        fps.render(screen, 0, 150, 'Initial: ' + str(round(first_distance, 4)))
                    else:
                        screen.fill((128, 128, 128), (0, 125, width, 25))
                        screen.fill((128, 128, 128), (0, 150, width, 25))
                        screen.fill((128, 128, 128), (0, 125, width, 25))
                        screen.fill((128, 128, 128), (0, 102, width, 25))
                        fps.render(screen, 0, 125, 'TSP: Best path found: ' + str(round(shortest_distance, 4)))
                        tsp_timed_out = False
                        fps.render(screen, 0, 150, 'Initial: ' + str(round(first_distance, 4)))
                        fps.render(screen, width/2, 102, 'Searched paths ' + value + ' times.')
                        fps.render(screen, width/2 + 30, 125, 'Distance: ' + str(round(shortest_distance, 4)))
