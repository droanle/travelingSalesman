...
info_string2 = "S toggles everything, but doesn't delete any nodes. T runs a brute force TSP solver."
        ...
        if show_elements:
            ...
            if tsp_ran:
                ...
                for path in itertools.permutations(nodes):
                    ...
                    screen.fill((128, 128, 128), (0, 80, width, 25))
                    fps.render(screen, 0, 80, 'TSP runtime: ' + str(round(ran, 4)) + 's')
                    screen.fill((128, 128, 128), (0, 102, width, 25))
                    fps.render(screen, 0, 102, 'Initial distance: ' + str(round(first_distance, 4)))
                    ...
                    fps.render(screen, width/2, 80, 'Searched paths ' + str(times) + ' times.')
                    fps.render(screen, width/2, 102, 'Distance: ' + str(round(shortest_distance, 4)))
                    if(ran > tsp_limit):
                        tsp_timed_out = True
                        break
                ...
                if tsp_timed_out or exit_tsp:
                    screen.fill((128, 128, 128), (0, 102, width, 25))
                    fps.render(screen, 0, 102, 'TSP: forced exit with best path as ' + str(round(shortest_distance, 4)))
                    tsp_timed_out = False
                else:
                    screen.fill((128, 128, 128), (0, 102, width, 25))
                    fps.render(screen, 0, 102, 'TSP: Best path found: ' + str(round(shortest_distance, 4)))
                    tsp_timed_out = False
